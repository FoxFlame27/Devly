...
| Email codes | `src/lib/auth/challenge.ts`, `src/lib/email/`, `src/lib/client/supabase.ts` | When `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` are set, the browser asks Supabase Auth to email a 6-digit OTP and verifies it; the resulting token is checked server-side against Supabase's `/auth/v1/user` (publishable key only, no service-role key) and Devly issues its own session cookie. Otherwise: | Signup and login send a 6-digit code (Resend when `RESEND_API_KEY` is set; otherwise the code is printed to the server log and shown on screen in development). Codes are stored hashed, expire after 10 minutes, and allow 5 attempts. Set `EMAIL_VERIFY=false` to turn this off. |
